<template>
    ...
    <div class="fixed top-4 right-4 z-50 flex gap-2">
        <button @click="voltarParaInicial"
            class="bg-slate-600 hover:bg-slate-700 text-white font-bold py-2 px-4 rounded-lg shadow-md transition-all text-sm flex items-center gap-1">
            ← Voltar
        </button>
        <button @click="salvarFicha"
            class="bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2 px-5 rounded-lg shadow-md transition-all text-sm flex items-center gap-1">
...
import { updateFichaCalculations, calcularDP } from '@/utils/rulesEngine';
...
export default defineComponent({
    ...
    methods: {
        carregarDadosFicha() {
            // Tenta buscar os dados salvos específicos usando a ID da rota
            const dadosSalvos = localStorage.getItem(`rpg_ficha_${this.id}`);
            if (dadosSalvos) {
                this.ficha = JSON.parse(dadosSalvos);
            }
        },
        salvarFicha() {
            // 1. Salva o estado completo da ficha atual
            localStorage.setItem(`rpg_ficha_${this.id}`, JSON.stringify(this.ficha));

            // 2. Atualiza o índice global de fichas para exibição na Home
            const bancoGeral = localStorage.getItem('rpg_fichas_index');
            let fichasIndex = bancoGeral ? JSON.parse(bancoGeral) : [];

            const resumoAtualizado = {
                id: this.id,
                nomePersonagem: this.ficha.nomePersonagem,
                nivel: this.ficha.nivel,
                raca: this.ficha.raca,
                profissao: this.ficha.profissao,
                campanha: this.ficha.campanha
            };

            const indexExistente = fichasIndex.findIndex((f: any) => f.id === this.id);
            if (indexExistente !== -1) {
                fichasIndex[indexExistente] = resumoAtualizado;
            } else {
                fichasIndex.push(resumoAtualizado);
            }

            localStorage.setItem('rpg_fichas_index', JSON.stringify(fichasIndex));
            alert('Ficha salva com sucesso no LocalStorage!');
        },
        voltarParaInicial() {
